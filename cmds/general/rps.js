...
module.exports.run = async (client, message, args) => {
  const player = toValue[args[0]]
  const bot = values[Math.floor(Math.random() * values.length)]
  let content
  ...
  if (args[0] === null) { content = `${title}...\n${nothing}\n${hint}` } else if (player == null) { content = `${anyTitle}\n\n${shoot(args[0], 'volcano')}\n\n${anything}` } else if (player == 'volcano') { content = `${anyTitle}\n\n${shoot('volcano', 'volcano')}\n\n${draw}` } else if (eggs.includes(player)) { content = `${title}\n\n${shoot(player, bot)}\n\n${lizardSpock}` } else {
    content = `${title}\n\n${shoot(player, bot)}\n\n`
    if (beats[player] === bot) { content += playerWin } else if (beats[bot] == player) { content += botWin } else { content += draw }
  }
